...
| R36 abuse detection and identity bans | NOT STARTED | nothing, R19 is done | no |
...
6. Emit the ban ops on impose and on lift, per decision 6. **Smaller than decision 6 says in one way and larger in another.** Smaller: R13 already landed **two** variants, `AuthOp::Banned` and `AuthOp::BanLifted` (`audit.rs`, both documented "Produced by R36"), so the variants exist and only the emission is missing, and decision 6's "a fourth `op` value" was written before R13 shipped. Larger: **`SessionManager` has no audit hook at all.** The hook lives on `AuthService` (`authn/service.rs`, a `OnceLock<AuditHook<S::Id>>` with a private `record`), and bans are detected in `SessionManager`, so one has to be added there in the same shape rather than an existing method being called.
