...
| 14 | R14 | **Unblocked since R5b landed, and still conditional.** R0's lock-wait fraction is zero at both subscriber counts, so that half of the trigger says no. The other half is now readable and step zero of the phase is reading it: warranted only if per-event work still grows with subscriber count now that the authorization cost is gone |
| 15 | R6 | **Unblocked, R5b landed 2026-08-14.** It was hard-blocked rather than cost-blocked, so nothing else stood in the way |
| 16 | R7 | Needs R6. R4 is done |
| 17 | R9 | **Unblocked, R5b landed 2026-08-14.** Mechanical, and last |
| 18 | R27 | Needs R6 for the incremental move-in and move-out. **The subql half is built**, landed as `0dac842` and `fcf7d83`. Compiling the filter needs the query set known in advance, which R27 now establishes for itself since R22 is deleted. Buildable before R6 only in a form that resyncs on every dependency change |
...
| any | R15 | Off the critical path. All five diesel proposals are merged upstream, `wal_checkpoint` last on 2026-08-14. What it waits on is a lock change rather than a request: the fork's `future` branch has to be rebased on upstream `main` before the pin reaches the fifth |
...
| done | ~~R40~~ **BUILT** | Replica policy enforcement wired into sync. Landed 2026-08-15. The rename is on the client at both sync boundaries, the map is a build artifact the client is refused without, and the demo's table carries a real policy. Its browser evidence is blocked by an upstream pg2sqlite defect, written up and not worked around |
...
| R14 dispatch-loop cost | NOT STARTED | nothing. R5b landed, so the counter test can be reread. Its lock-wait half already reads no | no |
| R6 two-check form | NOT STARTED | nothing, R5b landed | inherited |
| R7 revocation teardown | NOT STARTED | R6, R4 is done | inherited |
| R9 permissive policy out of tests | NOT STARTED | nothing, R5b landed | inherited |
...
| R27 membership term in the subscription language | NOT STARTED | R6. **The subql half is built**, landed as `0dac842` and `fcf7d83`, with `term_compile.rs`, a `membership-term` feature, term slots in the bytecode and VM, and `tests/membership_term.rs` | no |
...
| R15 replica retention and trimming | NOT STARTED | a pin move. All five diesel proposals merged, `wal_checkpoint` on 2026-08-14 as `diesel-rs/diesel#5150`, but the pinned fork branch `future` at `ac4cdfc3` predates it | merged, pin pending |
...
**Blocked on R6 alone. The subql half is built**, landed as `0dac842` and `fcf7d83`, which recognise a bounded membership subquery, key its subscriber lookup, and serve it by narrowing which subscribers a changed row admits. Researched and decided in `docs/architecture/04-subscriptions.md`, sequenced rather than urgent.
...
**Status.** BUILT (2026-08-15). Every step is implemented and proven on the production path. One proof, the browser suite's local-write half, is blocked by an upstream defect recorded in `docs/upstream-pg2sqlite-instead-of-insert-drops-defaults.md`.
