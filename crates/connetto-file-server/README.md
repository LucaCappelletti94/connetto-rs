...
Content bytes never enter Postgres. Chunks live in a local filesystem directory
or any `object_store`-compatible backend (S3, `MinIO`, local). Manifests and
reference counts stay in three typed diesel-async tables. Upload and download
are gated by compact Ed25519-signed tickets.
...
A crashed upload (intent without commit) never serves. Its chunks are cleaned
by the mark-sweep collector whose grace window matches the ticket lifetime.
