...
//!
//! The watermark keys on `(user_id, session_id)`: `user_id` is the deployment's
//! typed distributed `Id` (so it can foreign-key the users table) and
//! `session_id` is the connetto-minted session id from the verified access
//! token. The whole `INSERT ... ON CONFLICT (user_id, session_id) DO UPDATE SET
//! last_seq = GREATEST(...)` statement is laundered as one executable associated
//! type ([`Upsert`](ConnettoWatermarkSchema::Upsert)), named concretely in the
...
where
    // The plain `SELECT last_seq WHERE user_id = ? AND session_id = ?` read.
...
{
    /// The deployment's typed distributed user id, the `user_id` column value.
    ...
    /// The opaque `user_id = ? AND session_id = ?` predicate.
    type WmPk: Send;

    /// The whole `INSERT ... ON CONFLICT (user_id, session_id) DO UPDATE SET
...
/// [`ConnettoWatermarkSchema`] impl, parameterized by the deployment's `Id` type
/// and its diesel SQL type.
///
/// This is a convenience default only: the table is the deployment's to define
/// however it likes (a different name, a real foreign key on `user_id`, extra
/// columns), and [`ConnettoWatermarkSchema`] is the real contract, implementable
/// by hand against any table. The reference SQL is in
...
macro_rules! connetto_watermark_table {
    ($id:ty, $id_sql:ty) => {
        diesel::table! {
            /// The durable per-session mutation watermark: the deployment's
            /// typed user id, the connetto-minted session id, and the highest
        ...
        impl $crate::watermark_schema::ConnettoWatermarkSchema for ConnettoWatermark {
            ...
            type Upsert = diesel::helper_types::Set<
                diesel::helper_types::DoUpdate<
                    diesel::helper_types::OnConflict<
                        ...
                        >,
                        (_connetto_mutations::user_id, _connetto_mutations::session_id),
                    >,
                >,
                diesel::dsl::Eq<
            ...
            fn watermark_upsert(
                ...
                diesel::insert_into(_connetto_mutations::table)
                    ...
                    .on_conflict((_connetto_mutations::user_id, _connetto_mutations::session_id))
                    .do_update()
                    .set(
            ...
            fn wm_pk(user_id: $id, session_id: String) -> Self::WmPk {
                ...
                _connetto_mutations::user_id
                    .eq(user_id)
                    .and(_connetto_mutations::session_id.eq(session_id))
            }
        }
    };
}
