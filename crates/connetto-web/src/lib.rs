//! Browser platform for the connetto client on wasm32.
//!
//! The crate root implements [`BrowserSocket`], a [`Transport`] over
//! `web_sys::WebSocket` with the same framing as the native transport: every
//! message is a binary frame whose first byte is a wire tag from
//! `connetto_core::codec`, followed by the `MessagePack` payload. The futures
//! are not `Send` (they hold JS values), which is exactly what the
//! `MaybeSend` seam in `connetto-core` exists for.
//!
//! The modules build the dedicated-worker relay topology on top: a
