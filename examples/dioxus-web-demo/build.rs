...
/// Translate one tier, write its SQLite DDL to `ddl_out`, and report the
...
fn translate(documents: &[&str], ddl_out: &std::path::Path) -> Vec<String> {
...
fn main() {
    ...
    let synced_views = translate(
        &["../deployment/schema.sql", "../deployment/policies.sql"],
