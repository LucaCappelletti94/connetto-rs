...
**The two deletes above are different events and the wire must distinguish them. Decided (R29), now owned by R44.** The `op = Delete` case removes a row that no longer exists. The `old matches, new does not` case removes a row that still exists and merely left this subscription's window. The marker is the patchset op's own session-format indirect flag, set on synthesized departure deletes, so no frame or format changes (`15-replica-retention.md`, The one case predicates cannot answer).

**This section describes the intended delivery, not what the server does today. Corrected 2026-08-08.** The sentence here used to read "today both arrive as a delete and the client cannot tell them apart", and that was wrong in a way that matters: **no departure delete is produced at all.** The server encodes one patchset per CDC event and sends the identical bytes to every matched consumer, having merged the entered, still-matching and departed consumer lists into one, so a subscriber whose row departed receives the update as though the row still matched. The live consequence is over-retention rather than loss: the row stays in that replica and nothing ever removes it. The `op = Update` rules above are therefore the specification R44 implements, not a description of current behaviour.
