...
- **OpenFGA upkeep.** The per-row upkeep of the permission records lives in `subql`, driven from the change stream, because removing a record requires the value it was built from and `subql` is where both row versions are in hand. **Built (R5b, 2026-08-12).** `RowPolicy` calls `records_from_row` on the changed row to answer visibility, and `StoreUpkeep` runs `Shapes::diff` per changed row and writes it through `OpenFgaPolicy::apply` before the event is dispatched, with the replayed queries finished first. A change that moves a conditional tuple without moving its key, which every owner change does because the held-key arm is a condition over the wildcard, needs two calls rather than one, which subql `33ee3f8` fixed after connetto found it.
