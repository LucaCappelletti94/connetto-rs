...
| 8 | Pending-mutation resend. **Built (2026-09-24)** | client mutation queue | `client_seq` | Every pending write is resent on reconnect, and on a jittered timer after `Indeterminate` or a `RateLimited` naming a pending write. An `Indeterminate` write is never given up, like an offline one, and only the pending cap evicts it. |
