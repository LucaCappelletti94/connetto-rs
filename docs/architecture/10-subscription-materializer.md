...
- **OpenFGA upkeep.** The per-row upkeep of the permission records lives in `subql`, driven from the change stream, because removing a record requires the value it was built from and `subql` is where both row versions are in hand. **Decided (R5b).** The `rls2fga` per-row mapping this needs landed on 2026-08-07, so what blocks R5b is the subql half alone (`docs/upstream-subql-per-row-visibility.md`), which is unblocked and unbuilt.
