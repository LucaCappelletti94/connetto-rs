...
- **OpenFGA upkeep.** The per-row upkeep of the permission records lives in `subql`, driven from the change stream, because removing a record requires the value it was built from and `subql` is where both row versions are in hand. **Built (R5b, 2026-08-12).** `RowPolicy` calls `records_from_row` on the changed row to answer visibility, and `StoreUpkeep` runs `Shapes::diff` per changed row and writes it through `OpenFgaPolicy::apply` before the event is dispatched. A change that moves a conditional tuple without moving its key, which every owner change does because the held-key arm is a condition over the wildcard, needs two calls rather than one, which subql `33ee3f8` fixed after connetto found it.

  **The replay half is gone (R49, 2026-08-18), and this paragraph used to describe it.** A shape whose records span more than the changed row reported nothing removed and handed over a query to re-run, and re-running it only ever wrote what was still true, so a withdrawal left the grant in the store for ever and the change path kept answering yes. Startup now refuses any policy shape whose facts travel that way, which leaves no booting deployment able to produce one, so `FgaUpkeep::replay` and the key binding under it were deleted rather than left unreachable. The finding and its reproduction are in `upstream/subql-joined-shape-never-removes.md`, and the refusal has to be revisited when the upstream repair lands.
...
- **The write path end to end.** A client uploads a `MutationPatch`. The materializer authorizes the write (strict consistency preference, chosen by `subql` from the statement kind: a write always asks authoritatively, a read may take the cheap path. **Built (R5b, 2026-08-12)**, and connetto only configures the read side, since the write side is forced by the statement. See `08-authorization.md`), detects conflicts (the `updated_at` token per Q3.2), calls `subql.apply_diffset_bytes` in a transaction against its PG pool, and reports the outcome (per Q3.5 the CDC echo serves as the success ack, so only `MutationReject` and `MutationConflict` are dedicated messages). `client_seq` idempotency and the reply are the materializer's.
- **Oplog, reconnect, and catchup.** The retention-bounded oplog keyed by commit position, the catchup versus `FullResyncRequired` decision, and tombstones (`06-reconnect.md`) are session-bound server state. The catchup path carries the same two-version authorization obligation as the live path, and **built (R6)** it meets it from the stored `ChangeEvent`, which already carried both row images, so nothing was added to the oplog for it. It also carries the prepared compressed patch, so catchup reads bytes instead of rebuilding them per record per subscription, which is what takes `Materializer::encode_patch` off that path. **Decided (R16 part B)**, and it is what requires a subscription to outlive its socket by the retention window, because `Materializer::dispatch` builds a payload only when a consumer matches.
...
| 4 | Authorization call. **Built (R5b, 2026-08-12)** | materializer, auth filter | request id plus content hash | `OpenFgaPolicy` retries transport failures up to `DEFAULT_CONNECT_RETRIES` (2) times, then fails closed: no patch is delivered and no mutation accepted while the answer is unknown. `ControlMessage::DeliveryPaused { cause: PauseCause::AuthServiceUnreachable }` signals the outage to clients, and `MutationRejectReason::Indeterminate` signals a write refused for the same reason. |
...
| 6 | Server-side mutation apply (transient PG error, serialization failure). **Built (2026-09-24)** | materializer, mutation handler | `client_seq` | A pool checkout failure and the diesel kinds `SerializationFailure`, `ReadOnlyTransaction`, `ClosedConnection` and `UnableToSendCommand` retry with jittered backoff inside `SessionConfig::with_write_retry_budget` (5 s by default), and the apply reads the durable watermark in its own transaction first, so a commit whose answer was lost is acknowledged rather than applied twice. Past the budget the write is answered `MutationRejectReason::Indeterminate` and every later write of the connection waits behind it. Any other failure is `MutationReject` with the cause, and the client's optimistic write rolls back. A deadlock arrives as `Unknown` until the SQLSTATE reaches diesel, so it is one of those. |
...
| 8 | Pending-mutation resend. **Built (2026-09-24)** | client mutation queue | `client_seq` | Every pending write is resent on reconnect, and on a jittered timer after `Indeterminate` or a `RateLimited` naming a pending write. No pending write is ever given up or evicted, an `Indeterminate` one included, however many wait, and a record leaves the queue only when the server settles it. |
...
**Built (R32, 2026-08-09), and the response to an invalidated slot is smaller than the sentence above suggests.** The change feed reports nothing that distinguishes an invalidated slot from a dropped connection: every error is stringified and retried alike, and neither `subql` nor `pg_walstream` surfaces a SQLSTATE. So connetto detects the hole rather than the cause. Before opening the stream it reads where the slot will resume from and compares that against how far the reconnect log had already got. Everything the feed delivered was appended to the log before being acknowledged, so an ordinary reconnect resumes at or behind that mark and nothing happens. Ahead of it means changes occurred that the feed never delivered, whatever opened the hole: an invalidated slot the deployment recreated, a slot dropped under a running server, or a database restored from a backup.

**A declared epoch does two things, and needs both.** The log forgets everything through the resume point, which is retention with a different trigger rather than a new concept, and is what makes the returning client's question answer honestly without a second record to consult and keep in step. And every live connection is closed with `FatalErrorReason::ChangeStreamGap`, because a connection never asks that question again: reconnecting re-declares its subscriptions through the ordinary path, which rebuilds a running total from its source instead of repairing one that accumulated across the hole. Sending a bare resync notice instead is not available, since the client discards its rows the instant that frame arrives and expects the replacement immediately behind it.

**Accepted cost, stated because several servers sharing one database make it visible.** The log is shared, and `append` is written to tolerate several servers writing the same change. One server forgetting a stretch therefore also forgets it for servers that did receive it, whose clients then re-download rather than receiving only what they missed. That is wasted work and never lost data, because re-downloading is always correct, and it was chosen over a separate record of the hole, which would have been a third thing every deployment must create and a third input to the returning client's question.

### Deployment guidance for the slot

**Provision the slot and the publication before the server starts, and it will refuse to start without them, naming which is missing.** The slot is logical with the `pgoutput` plugin, and its name is unique across the whole cluster rather than per database, so two deployments sharing a Postgres instance need two names: `CONNETTO_SLOT` and `CONNETTO_PUBLICATION` set them, defaulting to `connetto_slot` and `connetto_pub`. A slot of the right name bound to a neighbouring database is refused rather than used, because it cannot be read from here.

**Set `max_slot_wal_keep_size`, and size it against the disk rather than against the workload.** Uncapped is the default and it is the dangerous setting: a decommissioned or long-crashed server holds the primary's journal until the disk fills, at which point writes stop for every application on that database and not only for sync. Capped, the same situation invalidates the slot instead, which costs every client a full re-download and costs the deployment nothing else. Pick a figure the primary can afford to lose to one stalled consumer, then watch the headroom: the `replication slot` log line carries `safe_bytes`, which is how much may still be written before this slot is invalidated, and the field is absent exactly when no cap is set.

**Watch `wal_status` alongside it.** `reserved` is healthy, `extended` and `unreserved` mean the slot is beyond its cap and living on borrowed time, and `lost` means it is already invalidated and the changes it was holding are gone. `active` is whether anything is reading: a slot retaining log with `active` false is the shape that fills a disk. The cadence is `CONNETTO_SLOT_LAG_SECS`, default 60 seconds, and `0` turns the watch off entirely, which removes the only warning that arrives before the cap trips.
...
2. ~~**Trigger executor concurrency.** How many concurrent re-executions per session and per server? A pool with a per-session cap plus a global cap is the obvious shape, but the numbers and the back-pressure behavior when the cap is hit need deciding.~~ **Decided: the shape stands, the numbers are an R0 output rather than a design choice, and the gap today is concurrency itself rather than its bound.**
   ...
   There is no pool and no cap. The `dispatched.triggers` loop in `SessionManager::dispatch_event` awaits `execute_scalar` one trigger at a time inside the fan-out loop, and a failure is skipped with `continue`. That is the same serial-await shape `08-authorization.md` records as the scalability wall `visible` used to be, in the same loop. R5b removed it for `visible` and not here, so the re-execution trigger loop is now the only surviving instance of the shape. Picking cap numbers before R0 measures the loop would be picking numbers for a structure the fan-out phase may replace.
...
4. ~~**Auth retry policy under outage.** If OpenFGA is down for minutes, should the materializer pause delivery (fail closed, conservative, may stall many clients) or degrade with a cached policy (riskier)? Decide before production.~~ **Built (R5b, 2026-08-12), fail closed.** No patch is delivered and no mutation accepted while the answer is unknown. Both wire additions are in the tree: `ControlMessage::DeliveryPaused { cause: PauseCause }` and `ControlMessage::DeliveryResumed` (typed signal that delivery is paused rather than quiet), and `MutationRejectReason::Indeterminate` (a reason that cannot be confused with `Unauthorized`, whose doc comment states the client MUST retry). Snapshots are unaffected, since they run on Postgres RLS. The consistency preference for reads versus writes is chosen by `subql` from the statement kind: connetto configures the read side and the write side is forced. See failure inventory row 4 for the retry count.
