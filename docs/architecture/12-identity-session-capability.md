...
**A commit is attributed to each key, not to the packed set. Corrected (fix/content-ticket-caller-binding, 2026-09-20).** The packed value exists for the one setting a policy reads, and nothing else may treat it as a name. When the content path tells the deployment's `connetto_set_content_state` who committed a file, it calls the setter once per key the caller holds, so a deployment storing that value as the owner ends with one row per key and the caller sees its own upload under any of them. Naming the packed list instead produced an owner that matched no single key and hid the file from the caller that uploaded it. A deployment setter is therefore called more than once for one commit and must be idempotent, which the `ON CONFLICT DO NOTHING` shape the chapter already recommends satisfies.
...
The replica file is named `prefix-sha256(canonical(user_id))` truncated to 128 bits (the hashing step inside `replica_db_name` in `crates/connetto-client/src/replica.rs`), unsalted and identical on every device. Anyone with live filesystem access can therefore **confirm** that a suspected account used this device by hashing a guessed id and testing for the file. The file names alone do not permit enumeration. The credential store now does: `RefreshTokenStore::accounts()` returns every account the store holds a token for. After R23, the store opens only under a key derived from a user-verified passkey, so reading that list costs a successful gate ceremony rather than mere file access. On a profile that never enrolled, the store opens under a stored device key and the list is as reachable as the file names are. Confirmation from a file name is still enough when the account name is already known, and the same name correlates one account across devices.
