...
**An application may ask again after time away. Decided (R94, 2026-09-25), not built.** The client builder's `Gate` carries one re-check grace. Absent, a launch unlocks once as above, which stays the floor. Set, the gate asks again when the application returns after being away longer than the grace, and zero means every return. connetto learns of leaving and returning itself on phones and in the browser, and through the Dioxus and Yew adapters or one call on the desktop, and measures time away from the two moments rather than with a timer, since a suspended iOS process runs none.
...
**Native mechanism: Decided (R51), not built.** `apple-native-keyring-store`'s `protected::Store` with `AccessPolicy::RequireUserPresence` is measured equivalent to biometry-any combined with device passcode on all three points (probe N3, at 1.0.1), including surviving a fingerprint-set change. The recorded decision to upstream the policy is discharged, since the crate already provides it. One unlock per launch comes from one `LAContext` evaluated once and passed on every keychain query (decided 2026-09-25, measured on an iPhone by R51's probe I5), which the crate does not pass. The change was requested of `apple-native-keyring-store` and `security-framework` on 2026-09-25, and R51 builds against the two fork branches through pinned patches until both are released, rather than keeping keychain code of its own. Until R51 wires it in, every native target reports a stored key with no verification, because reporting a gate connetto does not yet provide would be worse than reporting none. The packaging constraint applies when it lands: the data protection keychain needs a provisioned signed `.app` carrying the `keychain-access-groups` entitlement, and a bare signed binary is killed at exec by AMFI (rc 137), so custody reports platform-cannot for a development build.
...
**On by default wherever the platform supports it. Decided (R94, 2026-09-25), not built.** An application turns the gate off explicitly through the client builder, and a user can still dismiss the platform's prompt. Items stored before a gate existed stay ungated until rewritten, and how R51 and R52 rewrite them is theirs to decide.
...
**Android, built by R88 (2026-09-23).** The native client keeps the refresh token and the replica keys in `android-native-keyring-store` 1.0 rather than the stock `android-keyring` weighed above, which stores them in `SharedPreferences` encrypted under an Android Keystore key, and the replica is SQLCipher over vendored OpenSSL. The connection reports `Custody::Unverified(NoGate::Unsupported)`, because nothing on Android offers connetto a gate until R52 builds the Keystore key with user authentication required. On a Samsung Galaxy M52 5G (Android 13) and the API 35 emulator, sign-in provisions the key and opens the encrypted replica with no prompt.

**iOS, built by R88 (2026-09-24).** The native client keeps the refresh token and the replica keys in the Data Protection keychain, the only keychain iOS has, through `apple-native-keyring-store`'s `protected` store, and the replica is SQLCipher over CommonCrypto. The connection reports `Custody::Unverified(NoGate::Unsupported)`, the client's default the demo leaves in place, until R51 lands its gated keychain item. On an iPhone 15 Pro Max and an iPad (A16), both iOS 26.6.2, and an iOS 26.5 simulator, sign-in provisions the key and opens the encrypted replica with no prompt.
