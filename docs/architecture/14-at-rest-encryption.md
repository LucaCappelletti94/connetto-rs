...
**Browser wipe timing (Built).** The browser's replica connection lives inside the relay hub's pump for the worker's whole lifetime. A wipe cannot run while the connection is live, so `connetto_web::storage::mark_wipe_pending` records the replica and its exact persistent content namespace in `connetto-pending-wipes`. The next `boot_db_worker` removes that namespace recursively before destroying the replica key and files, then clears the marker only after both removals succeed so a failed boot retries the wipe. Legacy records without a namespace retain their earlier replica-only behavior.
