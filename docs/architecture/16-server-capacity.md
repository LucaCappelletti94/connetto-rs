...
**Amended (2026-09-20).** The reserved share is for callers whose handshake resolved anything, a login or a share key, not for identified callers alone. A key is connetto-signed and the permission behind it is a row the application can delete, so its holder is exactly as attributable and as cuttable as a signed-in one, and the anonymous allowance is for a caller that proved nothing. Keying the tier on the identity alone put a caller whose whole authorization comes from a key in the unreserved share, where it could be refused under load while presenting a credential the deployment issued.
