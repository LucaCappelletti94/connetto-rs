...
1. **The size of each pool.** Both are a library default nobody chose, and a reserve cannot be carved out of a number that was never decided. The number should come from measurement, which is what R0 counts.
