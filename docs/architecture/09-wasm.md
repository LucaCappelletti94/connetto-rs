...
4. ~~**Main thread read access**: should the main thread be able to query local SQLite directly (e.g. via a synchronous OPFS access in a `Worker`), or should all reads go through the worker message-passing API? The former is faster. The latter is architecturally cleaner.~~ **Decided (Q9.4):** All reads go through the worker that owns the database, via a custom `diesel_async` connection. Application code writes standard `diesel_async` queries and the worker transport is an implementation detail. (The recorded decision said `SharedWorker`, and the shipped worker is dedicated. The substance, that reads do not touch the database directly from a tab, is unaffected. See the note under question 1.) **Status split:** the requirement is delivered, the mechanism was superseded. No per-query forwarding connection exists. A tab runs an unmodified `ConnettoConnection` with an in-memory mirror (`Replica::Ephemeral`) over the relay (`RelayHub`, Q2.1 in `13-client-connection.md`), subscribes like any client, and application code writes standard synchronous diesel plus the typed live layer against the mirror, proven end to end by the wasm-smoke suites (`relay.rs`, `parity.rs`, `notes_fanout.rs`, `election.rs`, `failover.rs`). Reads never travel per query. A per-query forwarding layer stays parked for the one case the mirror does not fit, a tab wanting a dataset too large to mirror per tab, with no evidence of need today.
