...
Across a restart the countdown runs from the recorded stop moment, so a subscription already past its grace is not re-declared at launch. **Built (R29)**: `replay_subscriptions` re-declares only records still live. **The died-while-watching half is decided and not built, found 2026-08-08 while reconciling this chapter with the tree.** The design gives a subscription the app died still watching, which has no recorded stop moment, a fresh countdown from launch, so the UI has time to re-claim it as screens mount and an abandoned one retires. The code does not anchor that countdown: the startup seed leaves the stop moment empty, so such a record is re-declared on every attach until the same query is watched and dropped again. The cost is one leaked server subscription per crashed run whose query set changed, self-healing for a stable query set through re-claim. The accepted asymmetry stands once built: a query unwatched moments before a crash is protected for less time than one watched through it. **Owned by R45 step 1** (plan, R29 step 2 carries the full account).
