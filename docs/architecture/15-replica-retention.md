...
**A local write marks transient interest, and post-ack interest is always explicit.** Every local insert or update marks interest in its rows at capture time, the same way `subql`'s row tracking learns interest from parsed ops. The mark is in-memory bookkeeping over the durable pending queue (`_connetto_pending`): set at capture, cleared when the server acknowledges the mutation, rebuilt from the queue at boot. It is not a third retention mechanism, it is delivery bookkeeping, and after the ack the row lives or dies by watches and pins like any other. A typed `insert` (and likely `update`) surface shaped like `watch` makes the choice explicit at the write site: the plain diesel write path says "write this, I do not care about it afterwards", and the typed variant composes the write with a mark of interest through the existing mechanisms, a watch over the written row or a pin, never through hidden per-row state. Names, return types, and generated-key mechanics are decided in the phase that builds it (R29).
...
**This section is the specification for R44, not a description of the server today. Corrected 2026-08-08.** It reads as though a departing row already arrives as a delete addressed to one subscription. It does not: the server encodes one patchset per CDC event and sends the identical bytes to every matched consumer, so a subscriber whose row departed receives the update as though the row still matched, and nothing ever removes it. Everything below states what must be true once departures are announced, which is R44's work, and the reasoning is unaffected by the correction.
...
A deletion and a departure from a subscription's window would be indistinguishable on the wire, and the predicate cannot separate them because it is evaluated against a row that is leaving either way. (Today the question does not arise, because no departure is announced at all, per the correction above.) **The delete must therefore carry which it is.** A removed row applies unconditionally. A row that merely left this subscription's window applies only when no surviving predicate matches it. **Decided (R29): it carries it as the patchset op's own session-format indirect flag.** The server synthesizes a departure delete `indirect(true)` and a genuine delete direct, restoring at fan-out the distinction the per-subscription translation erases (a Postgres `DELETE` event fans out as deletes everywhere, a Postgres `UPDATE` event fans out as a delete only for the subscriptions the row left). The flag is native to the patchset format and already round-trips, so catchup and resync inherit it, no frame changes, and no protocol addition is needed after all. The convention is scoped to server-synthesized patchsets, and client-captured changesets keep the flag's native trigger-caused meaning.
