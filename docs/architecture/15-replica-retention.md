...
**Decided (R60, 2026-09-01), built 2026-09-02: the residual pass runs by default on a measured threshold, superseding the callable-only shape of the free-up-space pass (D4's trigger, not its trim semantics).** The client counts rows applied per application table since the last pass, one map bump per row from the update hook while a server patch applies, and compares their sum against a configurable threshold (`with_residual_threshold`, default 10,000 rows). At the crossing a typed `ClientEvent::TidyDue` fires once, re-arming when a pass resets the measure, and under the default `ResidualPass::Automatic` the connection runs `tidy` itself right after the apply that crossed, inside `ConnettoConnection` so the native client, the browser worker and raw connection users all get it with no wiring. `ResidualPass::Manual` supplants the default: the event still fires and the application calls `tidy` at its own moment, the default-on-with-opt-out pattern. D4's reasoning survives inside the policy: the pass still never evicts while the transport is down, the trim keeps its ratio gate, and the measure (per-table counts plus the freelist share) is queryable at any time through `residual_pressure`. Rejected: per-table thresholds (the pass sweeps the whole replica whichever table fires, so the precision changes nothing), triggering on the freelist ratio (it only grows after deletes, so pure accumulation never moves it), and the exact uncovered count (measuring it costs what the pass costs). Proven by `crates/connetto-client/tests/it/residual.rs`.

**Decided (R60, 2026-09-01), built 2026-09-02: the raw `unsubscribe` evicts before it forgets, and an offline cancellation defers rather than strands.** The subscription-end pass above always ran on the live teardown path, but the raw API deleted its record first, taking the table out of every later pass's scope and stranding the synced rows permanently. Connected, `unsubscribe` now evicts scoped to the departing subscription's coverage, then forgets, then sends the frame, the live teardown's order. Offline, where eviction is barred by the guard above, it ends the record instead of forgetting it, and the next connected attach retires it through the existing ended-record branch, evict then forget. An ended record is never replayed as a subscription, which preserves what forget-first bought. Proven by `a_raw_unsubscribe_reclaims_what_its_subscription_synced` and `an_offline_unsubscribe_defers_eviction_to_the_next_connected_attach` in `crates/connetto-client/tests/it/retention.rs`.
