...
| `SnapshotEnd` | Control frame: snapshot complete, carrying `sub_id` and the cursor at which the snapshot was taken |
...
| `MutationReject` | Mutation rejected: client sequence number + reason code. **Built (R5b, 2026-08-12)**: gains `MutationRejectReason::Indeterminate` for when the authorization service is unreachable. It must not reuse `Unauthorized`, which asserts the caller lacks permission when the truth is that the server cannot tell, and which makes a client stop retrying and possibly discard the mutation. The variant's doc comment states the client MUST retry. **Built (2026-09-24).** The server also answers `Indeterminate` for a write the database could not take within its write retry budget, and answers every later write of the connection the same way until the first one is settled, because the exactly-once watermark is one number and a later write applied first would acknowledge the earlier one unapplied. The client keeps an `Indeterminate` write pending with its rows, reports `ClientEvent::MutationDeferred`, and sends its pending writes again on a jittered timer. |
...
| `FullResyncRequired` | Server requires the client to re-snapshot a subscription. **Built (R8, extended by R7, R48, R58 and R73).** `FullResyncReason` carries five variants, one per producer: `CursorOutsideRetention` when a resuming cursor has fallen out of the retained window, `AuthorizationChange` when a grant reaching the subscription's table moved, `TableTruncated` when a table the subscription reads was emptied, `SnapshotInterrupted` when a paged read failed part way and is read again, and `CursorBeyondHistory` when a resuming cursor lies past where its timeline ended or cannot be read (`06-reconnect.md`, Failover). Adding a variant is a wire change: the enum has no forward-compatible fallback for an unknown value. |
...
| `FatalError` | Server is closing the session: reason code. **Built (R2, R8, R19).** Every variant names a close the server performs, there is no catch-all, and `crates/connetto-core/tests/wire.rs` guards that with a wildcard-free match. R2 wired `SessionRevoked` and `ConnectionSuperseded`, R8 sends `ServerShuttingDown` on SIGINT or SIGTERM by walking the connection registry, R19 added `RateLimited` with its `retry_after_ms` for a caller over its connection or credential-refusal limit, R32 added `ChangeStreamGap` for a change feed that resumed past what it delivered, R73 added `DatabaseTimelineChanged` for a database that moved to another timeline under a running server, and the client surfaces the reason as `ClientEvent::ServerClosed` instead of treating it as a protocol violation, so it backs off rather than dying silently. |
...
**Built (2026-09-13): a mutation's compressed bytes are metered per identity too.** Until then a write took only R39's reader-share permit, which protects the pool equally against everyone and records nothing about who caused the load, so a caller writing as fast as the pool admitted was absorbed on every burst and counted on none. A mutation now charges its patch bytes to a per-identity token bucket (`ThrottleConfig::with_mutation_bytes_per_identity`, default 64 MiB a minute, keyed by the session handle for a caller with no identity so unidentified runs never share one bucket) and over it draws the same `RateLimited` the reader share sends, naming the sequence and the wait until the bytes fit, so the client keeps the write pending and resends once. The charge comes after the reader permit, so a write the share defers has spent nothing. A patch larger than the whole window is rejected rather than deferred, since no wait would ever let it fit. And a deferral of either kind gates the connection: every later sequence is deferred behind it with the same deadline, because the watermark is one number and a later write applied first would advance it past the deferred one, whose resend the watermark check would then acknowledge unapplied. That gate closes a hole the reader-share deferral had carried since R39. A subscribe now takes the reader permit before its allowance, so one the share defers has spent nothing and its retry is served, while a probe that fails to resolve still pays. The default was chosen by symmetry with the upload meter rather than from traffic: every workload in the tree writes one-op patches of under 90 bytes and the busiest identity-minute is 5 KB, so the tree's tail cannot size it, and a `debug` line per mutation (`mutation received`, with `op_count` and `patch_bytes`) is the instrument a deployment sizes its own from. The upload and mutation meters are built from the one `ThrottleConfig` the `RequestGuard` holds: a second copy of that config used to reach the upload meter separately and was silently the default in every deployment.
