...
The delivered server-side delta family is `COUNT`, `COUNT(col)`, `SUM`, `AVG`, `VAR_POP`, `VAR_SAMP`, `STDDEV_POP`, and `STDDEV_SAMP`, seeded once through the connector then folded per CDC event, and since the 2026-08-25 subql adoption (R82) the `GROUP BY` variants of that family fold per group: the wire carries one keyed upsert per changed group, an emptied group as a keyed removal, and the group key stays opaque bytes end to end. A subscription whose groups outgrow the server-configured budget (default 1024) demotes to whole re-reads, logged and counted server-side and never told to the client. `MIN` and `MAX` are served by the re-execution tier the same adoption delivered: seeded under the caller's own read tier, re-read under the shared server bound when a change retires the extreme. See `10-subscription-materializer.md` for the server mechanics and `13-client-connection.md` for the client-query mechanism.
...
**Amended 2026-09-01 (R85, delivered):** the per-viewer statistic over rows the client does *not* hold is now served, by re-execution rather than by a shared accumulator. An aggregate registration subql refuses on an RLS table is re-registered with `database_reads_per_consumer()` under the asking viewer's own binding: the read setup appends `set_config` statements for the identity and the packed capability subjects to R81's budget statement, and every read the engine runs for that subscription executes as that viewer through the deployment's non-superuser reader role. Each session registers its own subql subscription, so results key by query and viewer with no new routing, a change re-executes each affected viewer's read and moves only their answer, an unidentified caller keeps the refusal (there is nobody to read as), and non-RLS folds keep the shared fast path untouched. The catalog DDL must carry the table's `ENABLE ROW LEVEL SECURITY` marker, which is what subql classifies from. Proof: `crates/connetto-server/tests/it/rls_computed.rs`.
...
**Amended 2026-09-01 (R84, delivered):** the client side is `watch_groups`, a `LiveGroups<K, V>` holding one typed entry per group. The wire delta carries `group_values_json` (decoded values in `GROUP BY` order beside the opaque canonical key), so the map key is typed without decoding key bytes; the values rest in `_connetto_aggregates.group_values_json` for offline rebuild, and a demotion to whole answers rebuilds through the same rested-row path, so the tier change never surfaces. Grouped `query.live(&client)` dispatches at compile time from diesel's group clause and projection SQL types, deriving scalar or tuple `K` and aggregate `V`; float, numeric, and JSON map keys have no typed impl. `LiveRows<R>` is the explicit full-replacement handle for wider re-executed row shapes. Proof: all three tests in `crates/connetto-client/tests/it/grouped_live.rs`; the Dioxus desktop demo exercises generic `use_live` with `COUNT(*) GROUP BY quantity`.
...
7. ~~**Client schema for aggregate results**: the client stores aggregate results in local SQLite: what does the schema look like for GROUP BY results? A key-value table? A typed table generated from the spec?~~ **Decided (Q5.7):** Generic `_connetto_aggregates` table: `(sub_id TEXT, group_key BLOB, result_json TEXT, PRIMARY KEY (sub_id, group_key))`. The application reads results via a custom Diesel connection that deserializes `result_json` into `T: serde::DeserializeOwned`. No per-subscription DDL is generated. **Amended 2026-08-26 (R83, delivered):** built, with the key amended to query identity, `(query_id INTEGER REFERENCES _connetto_query(id), binds BLOB, group_key BLOB, result_json TEXT, updated_at INTEGER, PRIMARY KEY (query_id, binds, group_key))`. The `sub_id` key could not be found again after a restart, because wire subscription ids are minted fresh each run, so the row is keyed by the query text and binds the subscription already persists. The requirement is unchanged.
...
**Amended 2026-09-01 (R85, delivered):** the re-execution arm of the amendment above is built. The viewer's identity and capability subjects ride the registration's read setup as `set_config` statements, the refusal on an RLS table converts into a per-consumer re-read registration for an identified caller, and the two-viewer proof is `rls_computed.rs`. What remains true from the original decision: fold-tier accumulators never straddle viewers, and the replica stays the primary path for anything the client already holds.
