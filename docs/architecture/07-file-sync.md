...
> **Scope notice: `connetto-core` does not build file sync, and this chapter is not the design.** The normative record of the file-handling decisions is `18-file-handling.md`, which outranks this chapter wherever they disagree. R24 concluded on 2026-08-21: the boundary decided at Q7.1 to Q7.8 lives at the crate level rather than the repo level, so the file-handling code is built in this repository as file crates that depend on connetto and never the reverse, designed against the joint review at `https://github.com/LucaCappelletti94/file-system-review` (an earlier reference here named a `file-system` repository that does not exist). The design's reasoning and rejected alternatives are the ten positions in the plan's R24 section, executed by phases R64 to R69, and `FileStore` was deleted from `crates/connetto-core/src/traits.rs` by R66 on 2026-09-08, in favour of a signer seam.
>
...
Clients subscribe to this table like any other. When a file is created, updated, or deleted, the metadata change arrives as a `LivePatch`. The metadata update is the *signal* that triggers the client to fetch (or evict) content.
...
The choice is deferred with the chapter. The rest of this document describes the logical protocol without assuming the physical channel.
...
The file's `content_hash` in the metadata table is the hash of the ordered sequence of chunk hashes (a Merkle root or a simple hash-of-hashes, with the exact scheme to be decided when this phase is implemented).
...
1. Client receives a `LivePatch` for the `files` table indicating a new or changed file.
...
When a file is soft-deleted (metadata `deleted_at` is set), the client receives a `LivePatch` and may evict the local content. Hard deletion from the client's content cache happens at the application's discretion (the file might be kept offline even after deletion from the server).
...
~~All eight questions in this section are deferred with the chapter (Q7.1 to Q7.8). They will be addressed when file sync is implemented in a future phase.~~
