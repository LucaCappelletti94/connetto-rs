...
- Lets clients declare interest in specific queries (row-level or aggregate), delivering an initial snapshot and subsequent incremental updates
- Handles reconnect and offline gaps: clients track their position in the server's change log and catch up on reconnect, with a full re-sync when the log window is exceeded
...
1. ~~What is the initial target transport: WebSocket only, or also HTTP long-poll / SSE for environments where WebSocket is restricted?~~ **Decided (Q0.1):** WebSocket only. HTTP fallback is dead weight: transparent proxies are a non-issue with `wss://`, and serverless runtimes cannot hold the subscription state, CDC listener, or session registry the architecture requires.
2. ~~Should the client and server crates live in this repository or in separate crates published independently?~~ **Decided (Q0.2):** Single Cargo workspace in this repo, with crates published independently to crates.io. The current workspace layout (`connetto-core`, `connetto-server`, `connetto-client`, `connetto-web`, `connetto-dioxus`, `connetto-yew`, and the `connetto-test-harness` test crate) is the decision in action.
