...
connetto uses the Backend-For-Frontend (BFF) topology. connetto-server is registered with each identity provider as a confidential OAuth client and holds the provider client secret. The frontend, native or browser, is not an OAuth client at all. **Decided (R90, 2026-09-22), in the vocabulary of RFC 10017 (BCP 212):** the claim is exact at the provider boundary, where connetto holds the secret and provider tokens never reach any frontend, and at connetto's own boundary the browser shape is the BCP's BFF session, a refresh credential in an `HttpOnly` cookie beside a short-lived bearer access token the websocket keeps from worker memory.
...
Revocation invalidates the session in the store, and because the handshake checks session liveness, a revoked session is refused on the next connection even while its access token is still time-valid, and a live connection is dropped by the server holding it as soon as that server sees the invalidation. Revocation is therefore authoritative rather than bounded by the access-token lifetime. Its reach is instant in either store variant. A revocation the primary had not yet sent to a standby is lost if that standby is promoted, like any other write under asynchronous replication (`06-reconnect.md`, Failover).
...
## App login: the assertion exchange

**Decided (R91, 2026-09-18, unblocked 2026-09-20).** A bot (chapter 12, "Apps, bots and installations") authenticates the app, not a person, and never goes through a provider. It presents an EdDSA JWT signed by the app's private key to one route, `POST /auth/app-token`, on a library function `app_router` beside `auth_router` that the reference binary mounts on the auth listener when a registry is configured and a downstream mounts or wraps as it does share minting. The assertion carries `iss` the app id, `sub` the app id for a shared-view login or an installation id for an isolated one, which is RFC 7523's client-authentication shape, `aud` this deployment's `AuthConfig::issuer` so one key registered at two deployments cannot have an assertion for one accepted at the other, `sid` the session to resume when the bot has one, `jti`, `iat` and a short `exp`. The private key never crosses the wire. The route verifies the signature against `connetto_apps.public_key`, checks an isolated `sub` names an installation of that app, refuses while the app's owner, the installation's installer or the bot user itself is banned, refuses a `jti` seen within its `exp` window, and mints an ordinary access grant.

**The grant is a login grant with two claims more.** `GrantClaims::User` for the bot user, so the handshake's verifier, the store's liveness check and `FatalErrorReason::SessionRevoked` all apply unchanged, plus `gen`, the key generation the assertion was verified under, and `inst`, the installation id for an isolated login. Both are read by arithmetic, so the handshake builds the `Principal` with no store read, identity the bot user and capability `inst:<id>` when `inst` is present. The current generation and the accepted grant rows the filter needs come from a registry cache the session manager seeds at boot and maintains from the change log, never from a read on the handshake path.

**A bot keeps its session as a device keeps its refresh token.** The first login opens a session through the store and the route returns its id, which the bot's credential persists beside its replica. A later assertion naming `sid` mints against that session if it is live and belongs to that bot and that installation shape. The bot never presents a refresh token and never rotates one, so the theft response never arises. An exchange is the bot's provider check, it is the moment the app proved itself again, so it resets the staleness clock this chapter decides above, and under the absolute ceiling the store applies today a bot opens a fresh session at the ceiling and resnapshots once. The `refresh_token` the store mints with every session is discarded, and the store gains one operation, sliding the idle window without rotating a token. An isolated login without `sid` for an installation that already has a session replaces it, revoking the old one, since an installation has one session. A shared login without `sid` when one exists opens a second and the first dies at its ceiling.

**Rotation is a generation bump, not a revocation.** An owner updating the app's public key bumps the generation by trigger. A held grant below the current generation is refused at handshake, an assertion under the old key fails verification, and every live session of the bot is closed with a reason that tells the client to log in again keeping its `sid`, distinct from `SessionRevoked`, whose contract is that the session is gone. The sessions survive for the next login under the new key. Uninstall, app deletion and any of the three bans do revoke, through the store's existing path, a deleted app needing a revoke-by-user operation the store gains.

**Rate limiting** keys the exchange on the app id after the signature verifies and on the address before it, under the login throttle this chapter already has. Lifetime is `access_ttl`, renewed by re-signing, no new knob. The blast radius of one leaked private key is every bot of that app at every deployment where it is registered, bounded by rotation.
...
The browser topology from `09-wasm.md` forces custody. The dedicated DB worker owns the single server connection and is the only context with OPFS access, but a worker cannot navigate, so interactive login must happen in a tab. The access token is held by the worker, never retained by the page, and after R90 there is no second token to hold, the browser's refresh credential being an `HttpOnly` cookie no script context can read (**Decided (R90, 2026-09-22)**).

Login is a redirect (or a popup carrying the redirect, to avoid tearing down the leader tab and its worker) to connetto-server's login endpoint. connetto-server runs the whole OAuth flow server-side and returns connetto's own tokens to the worker. The browser is not an OAuth client, so it runs no PKCE and holds no provider token. The access token lives in worker memory and is attached to the handshake. The refresh token arrives in a per-account `HttpOnly` cookie that the browser attaches to the auth endpoints' requests and no script can read, so a cold start or a leader failover silently refreshes and resumes with no user interaction and no durable credential in browser storage. The cookie name carries the `__Host-Http-` prefix, whose rules the browser itself enforces, fixing `Secure`, `HttpOnly`, `Path=/` and no `Domain` (RFC 10017 section 6.1.3.2). Its `Max-Age` is the time left until the session lapses without a further refresh, set at login and again at every rotation, so the cookie survives a browser restart and never outlives what the server would accept, and the same endpoints demand a required custom header on every cookie-contract request, the CSRF defence RFC 10017 section 6.1.3.3.2 places on the `POST` where a form submission cannot arrive without it. The cookie renders `SameSite=Strict` unless the operator names `none` for a deployment whose app origin is a different site from its auth origin, and that setting earns the cookie on top-level cross-site navigation only. A page embedded in another site is a third-party context where `None` without `Partitioned` does not ride, so an embedded deployment needs CHIPS, which R90 does not build and no setting of `CONNETTO_AUTH_COOKIE_SAMESITE` substitutes for. The auth listener grants CORS credentials, which a `credentials: "include"` fetch needs, only to the origins `CONNETTO_AUTH_CORS_ORIGINS` lists, a loopback development page included (**Decided with the maintainer 2026-09-23**). A page it does not list is answered without them, so under `none` no page on the user's machine can spend the cookie and read the token it returns. Telling development from deployment by the listener's bind address was rejected, since the default `127.0.0.1` bind behind a same-host proxy would read as development.

The enforced invariant that makes worker custody a real boundary: the worker's message protocol has no path that ever emits its access token back to a tab. Page cross-site-scripting, while resident, can drive the app as the user, which is unavoidable for any in-page attacker, and R90 removes the rest: with the refresh credential in an `HttpOnly` cookie it sits outside every script-reachable storage, so there is no durable credential to read, use off-device, or keep after the tab closes, precisely the property RFC 10017 sections 8.5 and 8.6 tell an application it cannot obtain, not even with non-exportable `IndexedDB` keys, because a key that survives to the next session is bytes on the same device and one file copy takes key and value together. Worker custody bounds the blast radius to the live session rather than a portable, persistent account takeover.

**That invariant holds for the credentials and is qualified for the replica key, by `14-at-rest-encryption.md`.** The gate on locally stored secrets derives a key from a passkey, and `PublicKeyCredential` is `[SecureContext, Exposed = Window]`, so the derivation cannot happen in the worker and the key must cross from a tab. What crosses is a non-extractable `CryptoKey` rather than bytes, so script arriving after the unlock can ask the browser to use it while the page lives but cannot export it, persist it or take it off the device, which is the property this paragraph claims. The exception is narrow and real: the raw bytes exist in page script between the assertion resolving and the import, and script already resident at that instant obtains exportable material. The window is irreducible because the extension returns a buffer and no path leads from a PRF result directly to a key object. R90 leaves the access token still never leaving the worker, and the browser holds no refresh token at all.
...
**One primary, and standbys for failover. Decided (R73).** A deployment reads and writes one primary, and a standby promoted after a failure carries the database store with every other table, so sessions, refresh tokens and revocations survive the promotion up to replication lag. `06-reconnect.md` holds the recipe and what clients experience. The access token's authenticity is checked locally with connetto's public key, which is not a secret. Several writable databases kept in sync, multi-master, is out of scope, because connetto rests on one ordered change log. A single-server deployment, in-memory or database, revokes with a local lookup.

**Revocation.** Revocation invalidates the session, and the handshake liveness check makes it authoritative: a revoked session is refused even with a time-valid access token, and a live connection is dropped when its server sees the invalidation. The reach is a local instant operation. The access-token lifetime is a re-auth cadence, not the revocation bound, and a deployment needing tighter reach shortens it. connetto adds no separate revocation channel.
...
5. **Browser client acquisition.** Login redirect or popup to the backend, worker custody of the access token, the refresh token in a per-account `HttpOnly` cookie, silent resume on cold start and leader failover through the cookie, and the message-protocol invariant that tokens never leave the worker.
...
- **The browser's refresh credential is an `HttpOnly` cookie and the native one stays in the OS keychain. Decided (R90, 2026-09-22).** The three auth endpoints serve both shapes through one service, the browser contract being the required custom header with a per-account `__Host-Http-` cookie and a body that names its account and carries no token, per RFC 10017 sections 6.1.3.2 and 6.1.3.3. The cookie's `Max-Age` is the time left until the session lapses, set again at every rotation (decided 2026-09-22), and CORS credentials go to the listed origins alone (decided 2026-09-23).
- **One pluggable auth store, two variants.** An in-memory store (single-server, ephemeral) and a database store (`DbAuthStore<S>` generic over a `ConnettoStoreSchema` trait, durable, carried by a promoted standby, connetto emitting no schema). The store holds connetto's sessions and refresh tokens and the user's retained provider tokens. Identity resolves through an `IdentityResolver`: a deterministic `(iss, sub)` to UUID v5 mapping by default (the in-memory store), or the deployment's own users-table mapping (creating or linking rows) in the database case, the latter supporting account linking.
...
- The two store variants are not only a storage-location choice, they are the ephemeral versus durable choice, and the deterministic-mapping versus account-linking choice. A single-provider or simple deployment uses the in-memory store and deterministic mapping. The moment one human may hold several logins, or the deployment needs durability or a standby to fail over to, the database store is required, which is why both ship.
