...
**Built, defective.** The trust-the-client-id line is gone from `run_handshake`, replaced by a call through `SessionVerifier`, and that closed the hole for any deployment that supplies a real verifier. It did not close the hole, because the default verifier reproduces exactly the old behaviour: `TrustingSessionVerifier` takes the string the client presents, uses it verbatim as the user id, and hashes it into a session id, and `SessionManager::with_oplog` installs it. So a caller still assumes any identity it names, that string still reaches `app.user_id`, and RLS still trusts it. The gap moved rather than closing, and it now looks closed, which is worse. Closing it for real means resolving each grant into a `Principal` from credentials the server can cryptographically verify, and having no default that skips the verification. R1 removes the permissive paths, R2 deletes the stand-in once the session layer has an identity of its own, and R3 makes the resolution a grant list.
...
**Decided (R3).** The concrete edit at `run_handshake`: replace `let auth_ctx = AuthContext::new(handshake.client_id.clone())` with a call to the session verifier that resolves `handshake.grants` into a `Principal`, and on failure (no grant resolved) send a `FatalError` and terminate before any subscription or catchup work.
...
The **access token** is short-lived and asymmetrically signed. **Decided (R8).** It carries only the identity (`user_id`). `tenant_id`, `roles`, and `claims` are deleted because nothing ever read them. The handshake trusts the identity from the signature alone with no store round-trip, and separately checks that the session is still live so a revoked session is refused even while the token is time-valid. It is verified once at handshake, so a healthy long-lived connection is not dropped when the token expires mid-session. **Decided (R3).** It is one grant in `Handshake.grants` rather than the single value in `Handshake.auth_token`, which is a wire change and a protocol version bump. The lifetime is short by default and server-configurable, and it is a re-auth cadence, not the revocation bound.
...
**Audit.** Authentication events (login success and failure, token issuance, revocation) reuse the `auth_events` table from `08-authorization.md`, with structured logging for the high-volume path, no new mechanism. **Decided, not built:** no crate declares `tracing` or `log` and no `src` uses either, so every claim about structured logging in these chapters describes an intended mechanism. Phase R12 builds it, and R3 is blocked on it because a refused grant is silent on the wire and the log line is what makes it loud.
...
- **Revocation is authoritative and propagates at store speed**, local and instant on a single server and at replication lag across a mesh, with no separate revocation channel. **Decided (R2).** `FatalErrorReason::SessionRevoked` is wired so a live connection is closed rather than only refused at its next handshake.
...
- **Decided (R3).** The handshake carries zero or more opaque grants, each resolved independently into an identity, capabilities, or a refusal. `Principal` holds an optional identity plus resolved capabilities. This supersedes the uncommitted `Credential::{Anonymous, Token}` enum.
