...
connetto uses the Backend-For-Frontend (BFF) topology. connetto-server is registered with each identity provider as a confidential OAuth client and holds the provider client secret. The frontend, native or browser, is not an OAuth client at all. **Decided (R90, 2026-09-22), in the vocabulary of RFC 10017 (BCP 212):** the claim is exact at the provider boundary, where connetto holds the secret and provider tokens never reach any frontend, and at connetto's own boundary the browser shape is the BCP's BFF session once R90 lands, a refresh credential in an `HttpOnly` cookie beside a short-lived bearer access token the websocket keeps from worker memory.
