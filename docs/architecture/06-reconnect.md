...
**Built (R6).** The catchup path carries the same two-version authorization obligation as the live path, and meets it from this table: the stored `event` is the whole `ChangeEvent`, so both row images are there, and that the previous image survives the JSON round trip is asserted by a test rather than assumed. `catch_up_row` routes through the same `transitions` call the live path uses, so a row this caller may no longer see replays as the plain delete that takes it back, and a deleted row this caller could never see replays as nothing. That catching up leaves the same rows as staying connected is proven by comparing two replicas over one event sequence.
