...
**Decided (R74, 2026-09-12): the clock rule and the lost-key path.** Certificate validity is checked against the wall clock with a stated skew tolerance, the one wall-clock check in the tree a peer cannot correct against a server. A device whose clock puts every certificate outside its window fails closed on every peer link and surfaces one typed event naming the cause, so an expedition device whose clock drifted off-grid is told why it sees no peers. A device whose custody record is gone, by a cleared keychain anywhere or by a reboot on Linux until R71 moves the session keyring's contents to custody that survives one, enrols again with a fresh key exactly as a first enrolment, and the dangling enrolment expires at its lifetime under the revocation-lag rule above. Provisional rows peers hold under the old key are adjudicated or retracted by the server's frontier like any other author's, since the device cannot sign for a key it no longer holds. On Linux the device key and the replica key die together at reboot, so until R71 the device-key loss adds no failure the replica has not already suffered.
