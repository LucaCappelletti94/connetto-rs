...
**Status split, amended 2026-08-22 by R30's conclusion.** The delivered scalar family had superseded this table for ungrouped aggregates: values ride `AggregateUpdate` pushes into an in-memory `LiveValue` (`None` until the bootstrap, frozen offline, `None` again after an offline restart). R30 reversed that split: the table is now the decided resting place for **every** server-computed result, scalars included (the single-row case with an empty key), grouped deltas upserting by the wire's dormant `group_key`, re-executed row sets keyed by position within a full-result replacement, so an offline restart shows the last synced value and the freeze becomes ordinary staleness. Grouped aggregates are no longer parked: the R30 section of `plans/master-implementation-plan.md` records the tier model (in-memory fold for delta-composable shapes under a server-configured group budget defaulting to 1024 with demotion to re-execution on overflow, hybrid per-group `MIN`/`MAX`, a general re-execution catch-all for everything else Postgres can run, and per-viewer re-execution on RLS tables). The memory obstacle this split used to record is answered by the budget plus demotion. Nothing is built yet, and the build is phased: R82 to R85 in the master plan for connetto's side, and the upstream request `docs/upstream-subql-grouped-and-reexecution-tiers.md` (a process artifact, never committed) for subql's, gated first on the refusal defect `upstream/subql-group-by-having-silently-dropped.md` (subql today silently accepts `GROUP BY`/`HAVING`).
...
The mechanism by which permission changes as Postgres rows reach the authorization service is now load-bearing for revocation: see the grant change entry under Q8.2 for how that propagates and what the response is. **Corrected 2026-08-07.** This paragraph said `rls2fga` generated whole-table queries alone, with nothing producing the change for one row. That was true when written and is not now: the crate describes a row's records without a database, through `RecordDescription` and the `records_from_row` evaluator, landed in full on its `main` at `d8f5dd7`. What remains is the consumer that drives those descriptions from the change stream, which lives in `subql` and is underway. So the dependency still blocks the phase that makes OpenFGA the change-path executor, but it is one half rather than two. See `08-authorization.md` under "An open dependency that blocks R5b", which is the owning statement.
...
**Decided (2026-09-14, before R69): every record carries its full span chain, and a log assertion in a test reads only the records its own span produced.** The stdout destination prints the enclosing spans outermost first beside the current span, so an operator reading a record from a nested task sees the connection and the session it belongs to. The server's merged integration target shares one process-global subscriber, and its log assertions were isolated by a lock that only the asserting tests took, so any concurrent test could satisfy a predicate. Each log-asserting test now runs its body under a `test` span carrying its name, instruments the session task its own helper spawns with that span, and captures only the records whose chain carries it, so the lock goes and the target stays parallel. A record from a task the library spawns internally carries no test span and cannot be asserted unless that spawn is instrumented, which none needs today. Rejected: a test-unique value in every predicate, which the tier transition record does not carry and every future assertion would have to remember, and serializing every test that can emit a matching record, which is most of the target. **Built 2026-09-16 in the `fix/log-capture-isolation` pull request.** `connetto_core::logging::install` sets `with_span_list(true)`, so a record carries the whole chain under `spans` beside the innermost span under `span`. In the server target a capture is opened by `with_capture`, which runs the test's body under a `test` span naming it and reads back only the records whose chain carries that name, and each connection helper instruments the session task it spawns with the span it was called under.
...
**Decision: `FULL` is a deployment requirement, checked at startup, refusing to start when a table the publication carries lacks it. Built (R6).** The change path needs the previous version of each changed row to distinguish a row that became invisible from one that was never visible. `REPLICA IDENTITY DEFAULT` records only primary key columns and records nothing when a table has no primary key. `FULL` records the previous values of all columns. Every existing test fixture already set it on the tables it streams and nothing checked it, so the change made an accident into a requirement. **Scoped to the publication rather than the database**, because connetto keeps its own bookkeeping tables in the same database and replicates none of them, so the database-wide reading subql ships would refuse a correctly configured deployment. A table altered after startup makes the server refuse to serve rather than choose between leaving a row on a device its owner may no longer see and handing its key to somebody who never could.
...
**Sequenced as two phases, R5a then R5b, and both are done.** Relocating the seam needed nothing and changed no behaviour, because the implementation behind the trait still used Postgres row-level security. Swapping in the authorization service needed a per-row mapping, which `rls2fga` landed on 2026-08-07, and the `subql` half that consumes it landed on 2026-08-11. Separating them let the unblocked half land first, put the measurement's instrumentation on a seam that then never relocated, and reduced the swap to substituting an implementation rather than restructuring a call path. The swap was a correctness prerequisite rather than a performance option: row-level security answers only about the row as it is now, and the two-version check under Q8.7 needs an answer about the row as it was, so no measurement could have vetoed it. **Delivered 2026-08-14**, with the round-trip count reading exactly zero on connetto's own policy shape at any audience size, and row-level security surviving as the snapshot and write executor and as the second opinion a parity mode compares against.
