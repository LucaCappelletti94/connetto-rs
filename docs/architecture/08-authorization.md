...
`rls2fga` used to generate whole-table queries alone, loading every permission record from scratch with nothing that produces the change for one row. It now describes a row's records without a database, so what remains is the consumer that drives those descriptions from the change stream, which is **underway rather than unbuilt** as of 2026-08-07. Until it lands, no answer on the change path has a stated freshness.

**Decided.** That upkeep lives in `subql`, driven from the change stream, because subql holds the replication connection and is the only place that sees every change with both row versions in hand, and removing a record requires knowing the value it was built from. `rls2fga` supplies the per-row mapping, and **it has since landed in full** (rls2fga `main` at `d8f5dd7`, as `RecordDescription` with the `records_from_row` evaluator beside it and `Translation::relations()` reporting per relation what is decidable from one row). What R5b still needs is the subql half that consumes it, `docs/upstream-subql-per-row-visibility.md`, **underway** on subql branch `feat/visibility-from-the-row`.

**connetto never depends on `rls2fga` directly, and does not need to.** It is absent from this workspace's `Cargo.toml` and `Cargo.lock` entirely and stays that way. It reaches the build only through `subql`, behind that crate's optional `visibility-records` feature, which is why the `no_std` constraint on the evaluator was worth insisting on upstream.

**Corrected the same day it was written, 2026-08-07.** This paragraph briefly recorded a decision that `subql` would re-export `RecordDescription.tables` so connetto's startup check could read it. That decision rested on a false premise, that the translator is the only thing which knows which tables a policy reads, and it is struck. The catalog already models policies: `DatabaseLike` exposes `policies()`, `has_policies()`, `rls_tables()`, `roles()` and `table_grants()`, and `PolicyLike` exposes `using_expression()`, `check_expression()`, `using_functions()` and `check_functions()`. So the startup refusal is computed from the catalog the server already parses, with nothing re-exported, no new dependency, and no catalog query. The rule that connetto does not name `rls2fga` survives, because nothing needs it, but it no longer has anything to do with that check.

### The replica enforces policy too, and that is three executors

**Decided (R40), 2026-08-07, recorded because it was the intent all along and no chapter said so.** The client's local SQLite replica carries the row-level-security policy, translated from the same Postgres policy text by `pg2sqlite`, which turns each policy-bearing table into a backing table, a view over it, and `INSTEAD OF` triggers. This is why substantial translation work went into that crate rather than the policies being dropped on the way to the client. **R40 is the phase that wires it into sync**, and until it lands the decision is recorded rather than usable.
...
**One build-time coupling follows, and it is the good kind.** A policy naming the caller cannot translate until the deployment says what the caller means locally, so `pg2sqlite` refuses with `SessionVariableMappingNotFound`, naming the pattern, and points at `with_session_variable()` and `with_session_user()`. That is a loud build failure rather than a trigger that silently evaluates the caller as nothing and admits every row. **No policy is rejected for being too complex.** The RLS translator raises exactly two errors, `SessionVariableMappingNotFound` and `RlsAuditTableNameRequired`, and the second only when audit behaviour is asked for without naming its table. Checked at both the revision the examples pin, `d0247132`, and upstream `main` at `a6f46dc`. An `UnsupportedPolicyPattern` variant existed at the pin, never constructed by anything, and has since been removed upstream. So the only gate is that every way a policy names the caller has a declared local meaning. The demo does not configure a mapping yet (`examples/wasm-smoke/build.rs` sets only the UUID options), so the first policy referencing the caller fails that build until one is added.

**The pin cannot move yet, and the reason is a chain worth knowing.** The examples pin pg2sqlite `d0247132` while `main` is `a6f46dc`, and the gap holds real RLS semantics fixes: permissive policies ORed and restrictive ANDed as PostgreSQL does, `WITH CHECK` defaulting to `USING`, and an RLS view denying every row when no policy applies to `SELECT`. Bumping was attempted on 2026-08-07 and reverted. pg2sqlite `main` needs a newer `sql-traits`, that newer `sql-traits` changes accessor signatures, and the change breaks the pinned `subql` at `c1f725a`. The adaptation already exists on subql's in-flight branch, whose own lockfile is on the newer `sql-traits` and whose `catalog_helpers.rs` is rewritten for it. **So the pin moves when that branch lands, not before.**

Attempting it also surfaced the cargo hazard this chapter warns about, live rather than hypothetical: `subql` names pg2sqlite `branch = "main"` and connetto names it `rev = "..."`, which are two sources to cargo, so the graph carries **two copies** of the crate. Harmless while neither passes pg2sqlite types to the other, and a trap the day one does.
...
**The fix exists upstream and is not wired in. That gap is R40.** `Pg2Sqlite::translation_manifest` exports the logical to physical map and `ParsedDiffSet::rename_tables` rewrites the bytes, and `rls_name_mapping.rs` proves both directions, the download landing in the backing table and a captured local write renaming back to the logical name. Neither symbol appears anywhere under `crates/*/src`: the production paths, `ConnettoConnection::apply_patch` and the upload capture, still work in plain names. **Nothing is broken today** because no synced table carries a policy yet, and everything breaks quietly on the first one that does, which is why R40 lands before anybody writes that policy. Also unverified: `clear_subscription_rows`, one function above `apply_patch`, issues a raw `DELETE` on the plain name for a full resync, which needs the generated `INSTEAD OF` delete trigger to survive. **Updated 2026-08-08 (R29):** that statement is no longer `DELETE FROM "{table}"`. It now carries a `WHERE NOT (..)` built from the surviving subscriptions' predicates, so R40 must check the trigger against a filtered delete, and the predicates themselves name plain columns of the logical table.
...
**Six of the eight kinds have a producer today**: `logged_out` from the logout endpoint, `session_revoked` from the application calling `AuthService::revoke`, `token_replayed` from the theft defence, `capability_minted` from `CapabilityIssuer`, and, since R36, `banned` from the abuse detector and `ban_lifted` from `BanStore::lift`. `permission_change` waits on R7 and `model_change` on R5b.
