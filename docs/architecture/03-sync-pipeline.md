...
**Built (R6): the filter consults both versions of the row.** `subql::visibility::transition::transitions` asks about the current version first and consults the previous one only when the current is absent or invisible to somebody, and connetto acts on the three answers it writes per watcher:

- **Deliver.** The patch this event produced, which for a subscriber that also left the subscription's own window is R44's marked departure notice.
- **Withdraw.** A plain unmarked delete carrying the key alone, so a row that left the caller's reach leaves their device too. A delete's own patch already is that, so only an update pays for a second fold. The caller cannot tell a withdrawal from a deletion, which is what stops the frame disclosing that the row still exists for others.
- **Nothing.** No frame at all, which is what stops a deleted row's key reaching a caller who could never see it.

Both directions were demonstrated before either was fixed, against a real client replica: a caller who lost access kept the row for ever, and a caller who never had access was told the row was deleted. A truncate is outside this: it names no row, so no question can be put, and the payload connetto folds for one carries no operations at all. R48 owns making a truncate empty the replica.

**Distinct, and built (R44): a predicate departure.** A row that stops matching a subscription's own `WHERE` clause while the caller may still see it is announced to exactly the subscribers it left, as a delete marked with the patchset op's indirect flag, and the client applies it only when no surviving subscription still covers the row. See `04-subscriptions.md`. **R6 retired that notice's exemption from the visibility question**, which had been taken while the two-check form was blocked: a subscriber who could never see the row used to receive its key whenever the row left their window. The transition now decides for every subscriber, and a departure notice survives only for one who may still see the row.
