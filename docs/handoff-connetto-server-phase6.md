...
- `connetto-client`: `ConnettoConnection` (renamed from `SyncClient`) with the explicit primitives `connect`, `subscribe`, `pump_one`, `push`, `conn` (the managed local SQLite connection for app reads and writes), `ping`, `close`, plus the ergonomic driver `flush`, `next_event`, and `take_changed`. An `on_commit` hook sets a dirty flag so `flush` and `next_event` auto-submit local writes with no explicit `push`, and `on_update` hooks feed `Reactive::changed_tables` for UI re-query. See `docs/architecture/10-client-connection.md`.
...
- Native client ergonomics (v1 landed): `SyncClient` is now `ConnettoConnection` with an `on_commit` dirty flag driving `flush` and `next_event` auto-submit and `on_update` hooks feeding `Reactive::changed_tables`, verified by the `loop_emu` `connection_autosubmits_writes_and_reports_changed_tables` test. Remaining: implement Diesel `Connection` and `LoadConnection` on it so queries run on the type directly, aggregate query routing, local rollback of a server-rejected write, and the WASM connection variants. See `docs/architecture/10-client-connection.md`.
